...
dpg.create_context()
dpg.create_viewport(title="Mp3-Player", large_icon="", small_icon="")
pygame.mixer.init()
...
def load_database():
    songs = json.load(open("songs.json", "r+"))["songs"]
    for filename in songs:
        dpg.add_button(label=f"{ntpath.basename(filename)}", callback=play, width=300,
                       height=30, user_data=filename.replace("\\", "/"), parent="list")
...
def update_slider():
    ...
    while pygame.mixer.music.get_busy():
        ...
        time.sleep(0.7)
    state = None
    dpg.configure_item("cstate", default_value=f"State:")
...
with dpg.theme(tag="base"):
    with dpg.theme_component():
        dpg.add_theme_color(dpg.mvThemeCol_Button, (130, 142, 100))
        dpg.add_theme_color(dpg.mvThemeCol_ButtonActive, (137, 142, 100, 95))
        dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (137, 142, 100))
...
with dpg.font_registry():
    monobold = dpg.add_font("MonoLisa-Bold.ttf", 12)
    head = dpg.add_font("MonoLisa-Bold.ttf", 15)
...
with dpg.window(tag="main", label="window title"):
    ...
    with dpg.group(horizontal=True):
        with dpg.child_window(width=200, tag="sidebar"):
            dpg.add_text("QWERTY", color=(137, 142, 255))
            dpg.add_spacer(height=2)
            dpg.add_spacer(height=5)
            dpg.add_separator()
            dpg.add_spacer(height=5)
            dpg.add_button(label="Add song", width=-1, height=28, callback=add_files)
            dpg.add_button(label="Add playlist", width=-1, height=28, callback=add_folder)
            dpg.add_button(label="Clear playlist", width=-1, height=28, callback=removeall)
            dpg.add_spacer(height=5)
            dpg.add_separator()
            dpg.add_spacer(height=5)
        ...
        with dpg.child_window(autosize_x=True, border=False):
            with dpg.child_window(autosize_x=True, height=50, no_scrollbar=True):
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_slider_float(tag="volume", width=120, height=15, pos=(160, 19), format="%.0f%.0%",
